    ...
    <header className="h-14 border-b border-white/40 dark:border-white/10 bg-white/60 dark:bg-gray-900/60 backdrop-blur-xl backdrop-saturate-150 flex items-center justify-between px-4 sm:px-6 shrink-0">
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="relative" ref={ref}>
          <button
            onClick={() => setOpen(o => !o)}
            className="relative text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800"
          >
            <Bell size={18} />
            {unread > 0 && (
          ...
          {open && (
            <div className="absolute right-0 top-10 w-80 bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-700 rounded-2xl shadow-2xl shadow-black/40 z-50 overflow-hidden">
              {/* header dropdown */}
              <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 dark:border-gray-800">
                <span className="text-gray-900 dark:text-white font-semibold text-sm">Notificações</span>
                {unread > 0 && (
                  <button
                    onClick={markAllRead}
                ...
                {notifications.length === 0 ? (
                  ...
                  notifications.slice(0, 8).map(n => (
                    <button
                      ...
                      className={`w-full text-left px-4 py-3 border-b border-gray-200 dark:border-gray-800 last:border-0 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors flex gap-3 ${!n.read ? 'bg-red-50/30 dark:bg-red-950/30' : ''}`}
                    >
                      <div className={`w-2 h-2 rounded-full mt-1.5 shrink-0 ${!n.read ? 'bg-red-400' : 'bg-transparent'}`} />
                      <div className="flex-1 min-w-0">
                        <p className="text-sm text-gray-900 dark:text-white leading-snug">{n.title}</p>
        ...
        <button
          ...
          className="text-gray-600 dark:text-gray-400 hover:text-red-600 dark:hover:text-red-400 transition-colors p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800"
          title="Sair"
        >
          <LogOut size={18} />
        </button>
