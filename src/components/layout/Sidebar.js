...
export default function Sidebar() {
  ...
  return (
    <aside className="hidden md:flex w-60 bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 flex-col py-6 shrink-0">
      <div className="px-5 mb-8 flex items-center gap-2">
        <Trophy className="text-red-500" size={22} />
        <span className="text-gray-900 dark:text-white font-bold text-lg">Nexus Arena</span>
      </div>
      ...
      <div className="px-3 pt-4 border-t border-gray-200 dark:border-gray-800 mt-4">
        <Link
          href="/configuracoes"
          className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"
        >
          <Settings size={18} />
          Configurações
        </Link>
