...
const statusConfig = {
  aguardando: { label: 'Aguardando', icon: Clock, classes: 'text-yellow-400' },
  ao_vivo:    { label: 'Ao vivo',    icon: Zap,   classes: 'text-green-400'  },
  encerrado:  { label: 'Encerrado',  icon: CheckCircle, classes: 'text-gray-400' },
}
    ...
    <div className={`bg-gray-900 border rounded-2xl p-5 transition-colors ${isLive ? 'border-indigo-700' : 'border-gray-800'}`}>
      ...
      <div className="flex items-center justify-between gap-4">
        <div className="flex-1 text-center">
          <p className="text-white font-semibold truncate">{teamA.name}</p>
          {isAdmin && !isOver ? (
            <div className="flex items-center justify-center gap-3 mt-2">
              <button onClick={() => changeScore('a', -1)} disabled={updating} className="w-7 h-7 rounded-full bg-gray-800 hover:bg-gray-700 text-white flex items-center justify-center disabled:opacity-40">
                <Minus size={13} />
              </button>
            ...
            <p className="text-4xl font-bold text-white mt-2">{score_a}</p>
          )}
        </div>
        ...
        <div className="flex-1 text-center">
          <p className="text-white font-semibold truncate">{teamB.name}</p>
          {isAdmin && !isOver ? (
            <div className="flex items-center justify-center gap-3 mt-2">
              <button onClick={() => changeScore('b', -1)} disabled={updating} className="w-7 h-7 rounded-full bg-gray-800 hover:bg-gray-700 text-white flex items-center justify-center disabled:opacity-40">
                <Minus size={13} />
              </button>
            ...
            <p className="text-4xl font-bold text-white mt-2">{score_b}</p>
          )}
        </div>
      </div>
      ...
      {poll ? (
        <div className="mt-5">
          <div className="flex items-center justify-between mb-2">
            <p className="text-gray-400 text-xs">{poll.question} • {total} {total === 1 ? 'voto' : 'votos'}</p>
            {isAdmin && (
              <button onClick={togglePoll} className="text-xs text-gray-500 hover:text-gray-300 transition-colors">
        ...
        <p className="text-center text-gray-600 text-xs mt-4">Sem enquete para este confronto</p>
      )}
    </div>
  )
}
