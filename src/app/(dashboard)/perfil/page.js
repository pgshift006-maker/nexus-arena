    ...
    <div className="max-w-[470px] mx-auto sm:px-4 py-6">
      <div className="px-4 sm:px-0">
        ...
        <div className="flex items-center gap-6 mb-5">
          ...
          <div className="flex-1 grid grid-cols-3 text-center">
            <div>
              <p className="text-white font-bold text-lg">{postsLoading ? '–' : posts.length}</p>
              <p className="text-gray-500 text-xs">publicações</p>
            </div>
            <div>
              <p className="text-white font-bold text-lg">{postsLoading ? '–' : totalCurtidas}</p>
              <p className="text-gray-500 text-xs">curtidas</p>
            </div>
            <div>
              <p className="text-white font-bold text-lg">{postsLoading ? '–' : totalComentarios}</p>
        ...
        {editing && (
          <form onSubmit={handleSave} className="bg-gray-900 border border-gray-800 rounded-2xl p-5 space-y-4 mb-6">
        ...
        <div className="border-t border-gray-800 pt-4 mb-1">
      ...
      {postsLoading ? (
        <div className="px-4 sm:px-0 space-y-3 mt-3">
          {[1, 2].map(i => (
            <div key={i} className="bg-gray-900 border border-gray-800 rounded-2xl p-4 animate-pulse h-24" />
          ))}
        </div>
      ) : posts.length === 0 ? (
